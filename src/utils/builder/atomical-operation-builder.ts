...
export class AtomicalOperationBuilder {
    ...
    async prepareStartSetup(): Promise<OperationSetup> {
        ...
        let walletPublicKeyBuffer = Buffer.from(walletPublicKey, 'hex'); // Assuming walletPublicKey is in hexadecimal format
        const fundingKeypairRaw = ECPair.fromPublicKey(walletPublicKey, NETWORK);
        const fundingKeypair = getKeypairInfo(fundingKeypairRaw);

        console.log("fundingKeypair", fundingKeypairRaw, fundingKeypair)
    ...
    async start(): Promise<any> {
        ...
        if (performBitworkForCommitTx) {
            ...
            // 假设所有签名操作都已经通过插件完成
                ...
                console.log("sleepEvery", this.options.sleepEvery)
        ...
        do {
            ...
            // @ts-ignore
            await window.unisat.signPsbt(psbt.toHex(), {
                toSignInputs: [
                    {
                        index: 0,
                        publicKey: walletPublicKey,
